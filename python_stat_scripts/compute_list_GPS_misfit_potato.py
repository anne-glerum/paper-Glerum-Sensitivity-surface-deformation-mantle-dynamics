...
with open("list_of_models_EGU2.dat", "r") as ifile:
  for name in ifile:
     ...
     misfit = 1. - np.sqrt(misfit_0.sum()/n_unmasked_points)
     ...
     print 'diff RMS: ',  np.sqrt(diff_vel_sqrd.sum()/n_unmasked_points)
     print 'GPS RMS: ', np.sqrt(GPS_vel_sqrd.sum()/n_unmasked_points)
     print 'percentage MRMS: ', np.sqrt(diff_vel_sqrd.sum()/(GPS_vel_sqrd.sum()*n_unmasked_points))*100.
