...
# This program compares ASPECT's point value velocity output with ref velocities
...
# It also writes to file the angle and velocity magnitude difference at each point.

# Remove Nb absolute motion
# -44.9 3.72 -0.1608
#omega = -0.1608
#Elat = 3.72 * PI / 180.
#Elon = -44.9 * PI / 180.
...
# Add Nb absolute motion
# -44.9 3.72 0.1608
#omega = 0.1608
#Elat = 3.72 * PI / 180.
#Elon = -44.9 * PI / 180.

# Add Eu absolute motion
# Eu : -38.63 21.61 0.1232
#omega = 0.1232
#Elat = 21.61 * PI / 180.
#Elon = -38.63 * PI / 180.

# Compute the cartesian vector components of the Euler pole
Ex = omega * 1e-6 * PI / 180. * math.cos(Elat) * math.cos(Elon)
Ey = omega * 1e-6 * PI / 180. * math.cos(Elat) * math.sin(Elon)
Ez = omega * 1e-6 * PI / 180. * math.sin(Elat)

# read in the file name of the predicted velocities to compare with ref
data_dir = '../EGU/K08_'
# read in a list of models that need to be processed
with open("list_of_models_EGU2.dat", "r") as ifile:
  for name in ifile:
     name = name.rstrip()
     if not name: continue
     data_file = data_dir+name+'/point_values.txt'
     print 'Comparing ', data_file, ' with GPS Nocquet in Doubrovine frame.'
     ...
     # add or remove a rotation to/from the aspect results
     aspect_data_masked['velocity_x'] = aspect_data_masked['velocity_x'] + (Ey*aspect_data_masked['evaluation_point_z']-Ez*aspect_data_masked['evaluation_point_y'])
     aspect_data_masked['velocity_y'] = aspect_data_masked['velocity_y'] + (Ez*aspect_data_masked['evaluation_point_x']-Ex*aspect_data_masked['evaluation_point_z'])
     aspect_data_masked['velocity_z'] = aspect_data_masked['velocity_z'] + (Ex*aspect_data_masked['evaluation_point_y']-Ey*aspect_data_masked['evaluation_point_x'])
     ...
     # compute angle for those data points that lie within the potato (i.e. that are not masked)
     ...
     # compute the absolute magnitude difference diff_magn for those data points that lie within the potato
     ...
     misfit_0 = (aspect_data_masked['velocity_x']*GPS_data_masked['velocity0']+aspect_data_masked['velocity_y']*GPS_data_masked['velocity1']+aspect_data_masked['velocity_z']*GPS_data_masked['velocity2'])/(GPS_vel*GPS_vel)
     misfit = 1. - misfit_0.sum()/n_unmasked_points
     ...
     # print misfit
     print 'misfit:', format(misfit, '1.3f')

     # print summary for easy pasting into exel
     print RMS_angle, RMS_diff_vel*100., misfit, RMS_angle/(angle_mean), RMS_diff_vel/(diff_vel_mean)

     # print angle and diff_magn for each point
     output_file = data_dir+name+'/GPS_misfit_potato.txt'
